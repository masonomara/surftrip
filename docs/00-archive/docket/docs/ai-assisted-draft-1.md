...
{{ Diagram: Three-part formula — "Industry Knowledge Base + Practical API Calls + Organizational Context = Delegatable Expertise" }}
...
{{ Screenshot of the Salesforce bot in Slack — first iteration in action }}
...
{{ Diagram: Channel architecture — Slack, Teams, Web, MCP funneling into unified ChannelMessage format and Durable Object }}
...
{{ Screenshot of a /demo/* endpoint in action }}
...
{{ Screenshot of CUD confirmation UX — human-readable summary: "Create Task: 'Review Contract' for Smith v. Jones?" }}
